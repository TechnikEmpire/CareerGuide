...
def test_qwen35_generator_defaults_target_small_cpu_runtime() -> None:
    ...
    assert settings.generation_model_artifact == "unsloth/Qwen3.5-2B-GGUF:UD-Q4_K_XL"
...
def test_setup_script_downloads_qwen35_q4_artifact() -> None:
    assert setup_local_models.GENERATOR_REPO_ID == "unsloth/Qwen3.5-2B-GGUF"
    assert setup_local_models.GENERATOR_LOCAL_DIR_NAME == "Qwen3.5-2B-GGUF"
    assert setup_local_models.GENERATOR_GGUF_PATTERN == "*UD-Q4_K_XL.gguf"
    assert "*UD-Q4_K_XL.gguf" in setup_local_models.GENERATOR_ALLOW_PATTERNS
...
def test_llama_cpp_server_example_caps_context_and_threads() -> None:
    ...
    assert model_config["model"].endswith("Qwen3.5-2B-UD-Q4_K_XL.gguf")
    assert model_config["model_alias"] == "unsloth/Qwen3.5-2B-GGUF:UD-Q4_K_XL"
