...
- скачивает generator GGUF в `models/Qwen_Qwen3.5-2B-GGUF/`
...
Default generator artifact — `bartowski/Qwen_Qwen3.5-2B-GGUF:Q4_K_M` из
`bartowski/Qwen_Qwen3.5-2B-GGUF`. Generated local server config ограничивает
context 8192 tokens и использует 4 CPU threads для текущего VPS с 8 GB RAM / 4 vCPU.
...
Если старый runtime уже был установлен, перед запуском Qwen3.5 GGUF выполните
...
поведение. Qwen3.5 требует `llama-cpp-python>=0.3.21`; более старые сборки
падают с `unknown model architecture: 'qwen35'`.
